import React from 'react';
import Counter from './client/components/Counter'; // Adjust path if Counter is in the same folder

export default function Main({ products, onAdd, onRemove, cartItems }) {
  // Guard clause: If products are missing, show a message instead of a blank page
  if (!products || products.length === 0) {
    return (
      <div className="container mt-5 text-center">
        <h3>No products found. Please check your Products.js file.</h3>
      </div>
    );
  }

  return (
    <main className="container mt-4">
      {/* Bootstrap Row for Grid */}
      <div className="row">
        {products.map((item) => {
          // Find if the item exists in the cart to get its current quantity
          const exist = cartItems.find((x) => x.id === item.id);
          const quantity = exist ? exist.qty : 0;

          return (
            /* col-md-3 ensures 4 items per row */
            <div key={item.id} className="col-12 col-sm-6 col-md-3 mb-4">
              <Counter 
                counter={item} 
                onAdd={onAdd} 
                onRemove={onRemove}
                quantity={quantity}
              />
            </div>
          );
        })}
      </div>
    </main>
  );
}